...
First we create a new user in the database.
Then we create a database named `nextcloud`.
We grant the new user all privileges to the new database.
...
been tampered with.
The signature should have had a signature so we can it, but then the signature
needs yet another signature, so we can trust... I'm lost.
...
Okay, so we just install `unzip` with the commands it shows.
...
> [!TIP]
> You can use (arrow up) to get previous commands, such that you don't have to
> retype it.
...
Then we need to move all the files it to the directory served by Apache web
server.
As the Apache2 Default Page told us, we should palce the files in `/var/www` directory.

```sh
sudo cp -r nextcloud /var/www
```

Apache run as a user called `www-data` with limited privileges, limiting the
damage that can be done by exploiting Apache.
...
Congratulations, you got your own self-hosted "cloud" storage.
It comes preloaded with a bunch of example files, so you can see what it can
do.
There are also clients available various operating systems
[here](https://nextcloud.com/features/?filter=Clients).
The clients allow easy synchronization of files from your devices.

> [!caution]
> Don't use this setup as is to sync your data.
...
or whatever.
It is just intended to let you do something cool with your server.
